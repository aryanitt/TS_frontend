...
export default function EmployeeProfileHeader() {
  ...
  const joinedAt = employee?.joiningDate || user?.createdAt;
  const role = employee?.role || user?.employeeRole || "Sales Executive";
  const department = employee?.department || "Sales";

  const actionLink = !onProfilePage ? (
    <Link
      to="/employee/profile"
        ...
        <div className="pt-9 sm:pt-12 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <div className="min-w-0 flex-1">
            <div className="flex items-start gap-2 flex-wrap">
              ...
              {employee?.id && (
                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-extrabold bg-slate-100 text-slate-500 uppercase tracking-wider shrink-0">
                  ID #{employee.id}
                </span>
              )}
            </div>
